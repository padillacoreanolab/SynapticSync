    ...
    def test_channel_order(self):
        with patch("sys.stdout", new=io.StringIO()):
            lfp_rec_0 = LFPRecording(
                "test subject 1",
                {"mPFC": 1, "BLA": 7, "vHPC": 31},
                EXAMPLE_RECORDING_FILEPATH,
                "",
            )
...
class TestH5File(unittest.TestCase):
    ...
    def test_1_save_rec(self):
        ...
        instance1.rms_traces = preprocessor.preprocess(traces=instance1.traces, threshold=4, scaling=instance1.voltage_scaling)
        LFPRecording.save_rec_to_h5(instance1, REC_PATH)
        ...
        try:
            ...
            self.fail(f"Could not verify H5 file format: {e!s}")
        try:
            with open(JSON_PATH) as f:
                json_data = json.load(f)
                self.assertTrue(isinstance(json_data, dict), "File is not a valid JSON file")
        except (json.JSONDecodeError, OSError) as e:
            self.fail(f"Could not verify JSON file format: {e!s}")

    def test_2_load_rec(self):
        instance1 = LFPRecording.load_rec_from_h5(H5_PATH)
        self.assertTrue(isinstance(instance1, LFPRecording))
        ...
        instance2.rms_traces = preprocessor.preprocess(traces=instance2.traces, threshold=4, scaling=instance1.voltage_scaling)
        attrs1 = vars(instance1)
        attrs2 = vars(instance2)
        self.assertEqual(
            set(attrs1.keys()),
            set(attrs2.keys()),
            "Instances have different attributes",
        )
        for attr in attrs1:
            if isinstance(attrs1[attr], np.ndarray):
                ...
                self.assertDictEqual(
                    attrs1[attr],
                    attrs2[attr],
                    f"Dictionary values differ for attribute {attr}",
                )
            else:
