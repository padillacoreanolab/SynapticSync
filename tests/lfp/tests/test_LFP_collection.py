...
from synapticsync.lfp.LFP_collection import LFPCollection
...
class TestLFPCollection(unittest.TestCase):
    ...
    def test_invalid_input(self):
        ...
        with self.assertRaises(TypeError):
            LFPCollection(None, None, None)  # type: ignore[arg-type]
...
class TestH5FileCollection(unittest.TestCase):
    ...
    def test_1_save_collection(self):
        ...
        collection_path = os.path.join(OUTPUT_DIR, "LFPCollection.json")
        ...
        for attr in attrs1:
            if isinstance(attrs1[attr], np.ndarray):
                ...
                self.assertEqual(
                    set(recs1.keys()),
                    set(recs2.keys()),
                    "Collections have different recording names",
                )
            ...
            if isinstance(attrs1[attr], np.ndarray):
                ...
                if attr == "merged_rec_path":
                    self.assertEqual(
                        Path(attrs1[attr]),
                        Path(attrs2[attr]),
                        f"Values differ for attribute {attr}",
                    )
                else:
                    self.assertEqual(
                        attrs1[attr],
